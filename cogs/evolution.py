...
class Evolution:
  def __init__(self,
    ...
    if mutation_probability == 'inv_genotype_length':
      ...
      num_unmutable_features = len([p for p in plausibility_constraints if p == '=']) if plausibility_constraints is not None else 0
    ...
    if self.evolution_type == 'p+o' and self.noisy_evaluations:
      raise ValueError("Using P+O is not compatible with noisy evaluations (you would need to re-evaluate the parents every generation, which is expensive)")
    elif 'age_reg' in self.evolution_type:
      print("Warning: using noisy evaluations but age regularized evolution does not re-evaluate the entire population every generation")
  ...
  def run(self):
    ...
    self.population.initialize(self.x, self.feature_intervals, self.indices_categorical_features,
      self.plausibility_constraints, self.init_temperature)
